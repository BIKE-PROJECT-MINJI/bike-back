...
- 배포된 V16 원본 SHA-256을 고정하고, V16 schema history에서 repair 없이 V36까지 validate/migrate
...
운영 Flyway 이력은 불변이어야 합니다. 사후 변경됐던 V16은 배포 원본 SHA-256으로 복원했고 중복 source course 보정은 V35에만 유지했습니다. V36은 AI 후보의 score/evidence/routing metadata를 영속화합니다. 별도 PostGIS schema에서 V1~V16 이력을 만든 뒤 현재 코드로 applied checksum을 검증하고 V17~V36을 `repair` 없이 적용했습니다. 이는 synthetic upgrade 계약이며 실제 운영 DB 배포 결과를 대신하지 않습니다.
