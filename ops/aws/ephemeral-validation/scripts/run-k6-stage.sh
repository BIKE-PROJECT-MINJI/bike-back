...
case "$STAGE" in
  ...
  baseline-10)
    readonly K6_SCRIPT='/scripts/bike-api.js'
    readonly K6_ENV='-e SCENARIO=baseline -e AUTH_AUTO_REGISTER=true -e BASELINE_TOTAL_VUS=10 -e BASELINE_RAMP_UP=20s -e BASELINE_HOLD=60s -e BASELINE_RAMP_DOWN=20s -e ADDRESS_SEARCH_EVERY_N_ITERATIONS=10 -e SYNTHETIC_CLIENT_IPS=true -e ERROR_RATE_MAX=0.01'
    ;;
  stress-25)
    readonly K6_SCRIPT='/scripts/bike-api.js'
    readonly K6_ENV='-e SCENARIO=stress -e AUTH_AUTO_REGISTER=true -e STRESS_TOTAL_VUS=25 -e STRESS_RAMP_UP=20s -e STRESS_HOLD=60s -e STRESS_RAMP_DOWN=20s -e ADDRESS_SEARCH_EVERY_N_ITERATIONS=10 -e SYNTHETIC_CLIENT_IPS=true -e ERROR_RATE_MAX=0.01'
    ;;
  burst-50)
    [[ "$ALLOW_HIGH_VUS" == "YES" ]] || {
