    ...
    private static final String HARNESS = """
        ...
        with tempfile.TemporaryDirectory() as d:
          ...
          case "$op" in
            ...
            ssm:get-command-invocation) test "$1" = --command-id && test "$3" = --instance-id && test "$4" = i-aaaaaaaa || exit 64; if test "$#" = 8; then test "$5" = --query && test "$6" = Status && test "$7" = --output && test "$8" = text || exit 64; case "${SSM_BEHAVIOR:-}:$2" in timeout:*) if test -f "$SSM_CANCEL_ATTEMPTED"; then case "$2:${SSM_CANCEL_FAILURE:-0}" in command-1:1) echo TimedOut;; command-1:*) echo Cancelled;; *) echo Success;; esac; else echo Pending; fi;; deploy-primary-cancel-failure:command-1) test -f "$SSM_CANCEL_ATTEMPTED" && echo TimedOut || echo Pending;; deploy-primary-cancel-failure:*) echo Success;; deploy-reconciliation-cancel-failure:command-1|deploy-reconciliation-stuck:command-1) echo Failed;; deploy-reconciliation-cancel-failure:command-2) test -f "$SSM_CANCEL_ATTEMPTED" && echo TimedOut || echo Pending;; deploy-reconciliation-stuck:command-2|deploy-primary-stuck:command-1) echo Pending;; deploy-primary-stuck:*) echo Success;; *) echo Success;; esac; elif test "$#" = 4; then case "$2:${SSM_BEHAVIOR:-}:${SSM_CANCEL_FAILURE:-0}" in db-1:*:*) printf '%s\\n' "$FAKE_DB_INVOCATION";; deploy-1:*:*) printf '%s\\n' "$FAKE_DEPLOY_INVOCATION";; roll-1:*:*) printf '%s\\n' "$FAKE_ROLL_INVOCATION";; command-1:timeout:1|command-1:deploy-primary-cancel-failure:*) printf '{"CommandId":"%s","Status":"TimedOut","StandardOutputContent":""}\\n' "$2";; command-1:timeout:*) printf '{"CommandId":"%s","Status":"Cancelled","StandardOutputContent":""}\\n' "$2";; command-1:deploy-reconciliation-cancel-failure:*|command-1:deploy-reconciliation-stuck:*) printf '{"CommandId":"%s","Status":"Failed","StandardOutputContent":""}\\n' "$2";; command-1:deploy-primary-stuck:*) printf '{"CommandId":"%s","Status":"Pending","StandardOutputContent":""}\\n' "$2";; command-2:deploy-reconciliation-cancel-failure:*) printf '{"CommandId":"%s","Status":"TimedOut","StandardOutputContent":""}\\n' "$2";; command-2:deploy-reconciliation-stuck:*) printf '{"CommandId":"%s","Status":"Pending","StandardOutputContent":""}\\n' "$2";; command-2:timeout:*|command-3:timeout:*|command-2:deploy-primary-cancel-failure:*|command-2:deploy-primary-stuck:*) printf '{"CommandId":"%s","Status":"Success","StandardOutputContent":""}\\n' "$2";; command-1:*:*|command-2:*:*|command-3:*:*) printf '{"CommandId":"%s","Status":"Success","StandardOutputContent":""}\\n' "$2";; *) exit 7;; esac; else exit 64; fi;;
            ssm:cancel-command) test "$#" = 2 && test "$1" = --command-id && test -n "$2" || exit 64; : > "$SSM_CANCEL_ATTEMPTED"; case "${SSM_CANCEL_FAILURE:-}:$2" in 1:command-1|primary:command-1|reconciliation:command-2) exit 73;; esac; : > "$SSM_CANCELLED";;
          ...
          env={**os.environ,'PATH':str(bindir)+':'+os.environ['PATH'],'BIKE_CD_TEST_ALLOW_UNPRIVILEGED':'1','APP_INSTANCE_ID':'i-aaaaaaaa','DEPLOY_S3_BUCKET':'bike-artifacts','APP_DEPLOY_DIR':'/opt/bike-back','APP_SERVICE_NAME':'bike-back','APP_PORT':'8080','HEALTHCHECK_URL':'https://bike.example/health','TARGET_DB_CREDENTIAL_PARAMETER':'/bike/db','AWS_DEPLOY_ROLE_ARN':'arn','GITHUB_SHA':'a'*40,'GITHUB_RUN_ID':'101','GITHUB_RUN_ATTEMPT':'1','GITHUB_OUTPUT':str(wd/'out'),'EXPECTED_DIGEST':'0'*64,'CURL_MODE':'normal','CURL_COUNT':str(wd/'curl-count'),'SSM_CANCELLED':str(wd/'ssm-cancelled'),'SSM_CANCEL_ATTEMPTED':str(wd/'ssm-cancel-attempted'),'SSM_SEND_COUNT':str(wd/'ssm-send-count'),'FAKE_DB_INVOCATION':db_evidence,'FAKE_DEPLOY_INVOCATION':deploy_evidence,'FAKE_ROLL_INVOCATION':roll_evidence}
          ...
          def deploy_control():
            (wd/'deploy-state-machine-command-id.txt').write_text('deploy-1')
            (wd/'deploy-state-machine-control.json').write_text(json.dumps({'commandId':'deploy-1','terminalStatus':'Success','cancelCommandStatus':'not-needed','reconciliationRequired':False,'reconciliationCommandId':'','reconciliationTerminalStatus':'not-run','reconciliationCancelCommandStatus':'not-needed','finalOutcome':'success'}))
          if scenario=='config-valid': assert run('Validate and encode deployment configuration',wd,env).returncode==0
            ...
            text=steps['Send and wait for pre-upload target DB gate']['run']; assert '--timeout-seconds 600' in text and 'aws ssm cancel-command --command-id "$command_id"' in text
            assert text.index('cancel-command') > text.index('seq 1 60') and 'Cancelled|TimedOut|Failed' in text and 'terminal.json' in text
            text=steps['Send and wait for deploy state machine']['run']; assert '--timeout-seconds 600' in text and 'poll_terminal "$command_id" 60 10' in text and 'poll_terminal "$command_id" 15 2' in text and 'deploy-state-machine-control.json' in text
            public=steps['Verify public health and readiness with compensation']['run']
            assert '--timeout-seconds 600' in public and 'aws ssm cancel-command --command-id "$rollback_id"' in public and 'public-rollback-terminal.json' in public
            assert 'file://ssm-reconcile.json' in public and public.index('public-rollback-terminal.json') < public.index('file://ssm-reconcile.json')
            assert 'cancel_status=failed' in public and 'public-rollback-reconciliation-control.json' in public and 'test ! -f "$run_dir/reconciliation.json"' in public
            deploy=steps['Send and wait for deploy state machine']['run']; assert 'file://ssm-reconcile.json' in deploy and deploy.index('deploy-state-machine-terminal.json') < deploy.index('file://ssm-reconcile.json') and 'aws ssm cancel-command --command-id "$reconcile_id"' in deploy and 'deploy-reconciliation-control.json' in deploy and 'deploy-state-machine-control.json' in deploy and 'cancel_status=failed' in deploy and 'reconcile_cancel_status=failed' in deploy
          elif scenario in ('ssm-deploy-primary-cancel-failure','ssm-deploy-reconciliation-cancel-failure','ssm-deploy-primary-stuck-reconciles','ssm-deploy-reconciliation-stuck-evidence'):
            deploy,previous,source,cdigest,proc=target(); commands,local=render_deploy(deploy,cdigest)
            pathlib.Path(local['SSM_CANCELLED']).unlink(missing_ok=True); pathlib.Path(local['SSM_CANCEL_ATTEMPTED']).unlink(missing_ok=True); pathlib.Path(local['SSM_SEND_COUNT']).unlink(missing_ok=True); log.write_text('')
            behavior={'ssm-deploy-primary-cancel-failure':'deploy-primary-cancel-failure','ssm-deploy-reconciliation-cancel-failure':'deploy-reconciliation-cancel-failure','ssm-deploy-primary-stuck-reconciles':'deploy-primary-stuck','ssm-deploy-reconciliation-stuck-evidence':'deploy-reconciliation-stuck'}[scenario]
            cancel_failure={'ssm-deploy-primary-cancel-failure':'primary','ssm-deploy-reconciliation-cancel-failure':'reconciliation'}.get(scenario,'')
            result=run('Send and wait for deploy state machine',wd,{**local,'SSM_BEHAVIOR':behavior,'SSM_CANCEL_FAILURE':cancel_failure},config(deploy))
            assert result.returncode!=0
            primary=json.loads((wd/'deploy-state-machine-control.json').read_text())
            reconciliation_control=json.loads((wd/'deploy-reconciliation-control.json').read_text())
            expected={'ssm-deploy-primary-cancel-failure':('TimedOut','failed','Success','not-needed','success'),'ssm-deploy-reconciliation-cancel-failure':('Failed','not-needed','TimedOut','failed','failure'),'ssm-deploy-primary-stuck-reconciles':('Unconfirmed','issued','Success','not-needed','success'),'ssm-deploy-reconciliation-stuck-evidence':('Failed','not-needed','Unconfirmed','issued','failure')}[scenario]
            assert (primary['terminalStatus'],primary['cancelCommandStatus'],primary['reconciliationTerminalStatus'],primary['reconciliationCancelCommandStatus'])==expected[:4]
            assert primary['reconciliationRequired'] and primary['reconciliationCommandId']=='command-2' and primary['finalOutcome']=='failure'
            assert (reconciliation_control['terminalStatus'],reconciliation_control['cancelCommandStatus'],reconciliation_control['finalOutcome'])==(expected[2],expected[3],expected[4])
            assert (wd/'deploy-state-machine-terminal.json').exists() and (wd/'deploy-reconciliation.json').exists()
            calls=log.read_text(); assert calls.index('file://ssm-deploy.json') < calls.index('file://ssm-reconcile.json')
            if scenario in ('ssm-deploy-primary-cancel-failure','ssm-deploy-primary-stuck-reconciles'): assert calls.index('ssm cancel-command') < calls.index('file://ssm-reconcile.json')
            else: assert calls.index('file://ssm-reconcile.json') < calls.index('ssm cancel-command')
            if scenario=='ssm-deploy-reconciliation-stuck-evidence': assert json.loads((wd/'deploy-reconciliation.json').read_text())['Status']=='Pending'
          elif scenario in ('ssm-cancel-before-promotion','ssm-cancel-after-symlink-promotion','ssm-cancel-during-restart','ssm-delayed-public-rollback','ssm-delayed-public-rollback-cancel-failure'):
            deploy,previous,source,cdigest,proc=target(); commands,local=render_deploy(deploy,cdigest)
            # The fake state machine yields Pending through the runner deadline,
            # then a terminal state only after cancellation is attempted.
            pathlib.Path(local['SSM_CANCELLED']).unlink(missing_ok=True); pathlib.Path(local['SSM_CANCEL_ATTEMPTED']).unlink(missing_ok=True); pathlib.Path(local['SSM_SEND_COUNT']).unlink(missing_ok=True); log.write_text('')
            ...
            if scenario in ('ssm-delayed-public-rollback','ssm-delayed-public-rollback-cancel-failure'):
              assert execute(commands,{**local,'FAKE_ARTIFACT':str(source)},deploy,proc).returncode==0
              candidate_path=deploy/('a'*40)/cdigest/'app.jar'; assert (deploy/'current.jar').resolve()==candidate_path
              document,public_local=public_document(deploy,cdigest)
              pathlib.Path(timed['SSM_CANCELLED']).unlink(missing_ok=True); pathlib.Path(timed['SSM_CANCEL_ATTEMPTED']).unlink(missing_ok=True); pathlib.Path(timed['SSM_SEND_COUNT']).unlink(missing_ok=True); pathlib.Path(timed['CURL_COUNT']).unlink(missing_ok=True); log.write_text('')
              cancel_failed=scenario=='ssm-delayed-public-rollback-cancel-failure'
              if cancel_failed: timed['SSM_CANCEL_FAILURE']='1'
              result=run('Verify public health and readiness with compensation',wd,timed,config(deploy))
              expected_terminal='TimedOut' if cancel_failed else 'Cancelled'
              assert result.returncode!=0 and (deploy/'current.jar').resolve()==candidate_path
              calls=log.read_text(); assert calls.index('file://ssm-public-rollback.json') < calls.index('ssm cancel-command') < calls.index('file://ssm-reconcile.json') < calls.rindex('ssm get-command-invocation')
              assert pathlib.Path(timed['SSM_CANCEL_ATTEMPTED']).exists() and pathlib.Path(timed['SSM_CANCELLED']).exists()==(not cancel_failed)
              assert (wd/'public-rollback-terminal.json').exists() and (wd/'public-rollback-reconciliation.json').exists()
              control=json.loads((wd/'public-rollback-control.json').read_text())
              assert control['terminalStatus']==expected_terminal and control['cancelCommandStatus']==('failed' if cancel_failed else 'issued'), control
              assert control['reconciliationRequired'] and control['reconciliationCommandId']=='command-2' and control['reconciliationTerminalStatus']=='Success'
              reconciliation_control=json.loads((wd/'public-rollback-reconciliation-control.json').read_text())
              assert reconciliation_control=={'commandId':'command-2','terminalStatus':'Success','cancelCommandStatus':'not-needed'}
              ...
              assert reconciliation_record['action'] in ('rollback','complete') and (deploy/'current.jar').resolve()==previous
              assert execute(document,public_local,deploy,proc).returncode!=0 and not (run_path(cdigest)/'public-rollback-status.json').exists()
              assert (deploy/'current.jar').resolve()==previous
            else:
            ...
            (wd/'public-rollback-control.json').write_text(json.dumps({'commandId':'roll-1','terminalStatus':'Success','cancelCommandStatus':'not-needed','reconciliationRequired':False,'reconciliationCommandId':'','reconciliationTerminalStatus':'not-run'}))
            invocation=json.dumps({'Status':'Success','StandardOutputContent':result.stdout})
          ...
          elif scenario=='control-missing-id-fails-closed':
            (wd/'public-rollback-control.json').write_text(json.dumps({'commandId':'roll-1','terminalStatus':'Success','cancelCommandStatus':'not-needed','reconciliationRequired':False,'reconciliationCommandId':'','reconciliationTerminalStatus':'not-run'}))
            assert evidence_step(env).returncode!=0
            verdict=json.loads((wd/'deployment-evidence/final-verdict.json').read_text())
            assert 'public-rollback:control' in verdict['evidenceValidationErrors'] and verdict['evidenceValid'] is False
          elif scenario=='control-missing-artifact-fails-closed':
            (wd/'rollback-command-id.txt').write_text('roll-1')
            assert evidence_step(env).returncode!=0
            verdict=json.loads((wd/'deployment-evidence/final-verdict.json').read_text())
            assert 'public-rollback:control' in verdict['evidenceValidationErrors'] and verdict['evidenceValid'] is False
