    ...
    private static final String HARNESS = """
        ...
        with tempfile.TemporaryDirectory() as d:
          ...
          case "$op" in
            ...
            ssm:get-command-invocation) test "$1" = --command-id && test "$3" = --instance-id && test "$4" = i-aaaaaaaa || exit 64; if test "$#" = 8; then test "$5" = --query && test "$6" = Status && test "$7" = --output && test "$8" = text || exit 64; if test "${SSM_BEHAVIOR:-}" = timeout; then if test -f "$SSM_CANCEL_ATTEMPTED"; then case "$2:${SSM_CANCEL_FAILURE:-0}" in command-1:1) echo TimedOut;; command-1:*) echo Cancelled;; *) echo Success;; esac; else echo Pending; fi; else echo Success; fi; elif test "$#" = 4; then case "$2:${SSM_BEHAVIOR:-}:${SSM_CANCEL_FAILURE:-0}" in db-1:*:*) printf '%s\\n' "$FAKE_DB_INVOCATION";; deploy-1:*:*) printf '%s\\n' "$FAKE_DEPLOY_INVOCATION";; roll-1:*:*) printf '%s\\n' "$FAKE_ROLL_INVOCATION";; command-1:timeout:1) printf '{"CommandId":"%s","Status":"TimedOut","StandardOutputContent":""}\\n' "$2";; command-1:timeout:*) printf '{"CommandId":"%s","Status":"Cancelled","StandardOutputContent":""}\\n' "$2";; command-2:timeout:*|command-3:timeout:*) printf '{"CommandId":"%s","Status":"Success","StandardOutputContent":""}\\n' "$2";; command-1:*:*|command-2:*:*|command-3:*:*) printf '{"CommandId":"%s","Status":"Success","StandardOutputContent":""}\\n' "$2";; *) exit 7;; esac; else exit 64; fi;;
            ssm:cancel-command) test "$#" = 2 && test "$1" = --command-id && test -n "$2" || exit 64; : > "$SSM_CANCEL_ATTEMPTED"; if test "${SSM_CANCEL_FAILURE:-0}" = 1; then exit 73; fi; : > "$SSM_CANCELLED";;
